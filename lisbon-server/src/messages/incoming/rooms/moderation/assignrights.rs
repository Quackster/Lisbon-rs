//! Mirrors `net.h4bbo.lisbon.messages.incoming.rooms.moderation.ASSIGNRIGHTS`.
...
impl MessageEvent for ASSIGNRIGHTS {
    ...
    fn handle(&self, player: &Player, reader: &mut NettyRequest) -> Result<(), String> {
        ...
        room.add_right(user_id);
        room.refresh_rights(&target);

        target.get_room_user().map(|room_user| room_user.set_needs_update(true));
        RoomRightsDao::add_rights(target.get_details(), room.get_data());
