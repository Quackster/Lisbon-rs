//! Mirrors `net.h4bbo.lisbon.messages.incoming.user.IGNORE_USER`.
...
pub struct IGNORE_USER;

impl MessageEvent for IGNORE_USER {
    ...
    fn handle_mut(&self, player: &mut Player, reader: &mut NettyRequest) -> Result<(), String> {
        ...
        UsersMutesDao::add_muted(player.get_details().get_id(), user_id);

        player.add_ignored_name(&username);
        player.send(&IGNORE_USER_RESULT::new(1));
