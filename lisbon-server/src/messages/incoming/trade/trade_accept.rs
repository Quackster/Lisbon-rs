//! Mirrors `net.h4bbo.lisbon.messages.incoming.trade.TRADE_ACCEPT`.
...
pub struct TRADE_ACCEPT;

impl MessageEvent for TRADE_ACCEPT {
    ...
    fn handle(&self, player: &Player, _reader: &mut NettyRequest) -> Result<(), String> {
        ...
        room_user.set_trade_accept(true);

        RoomTradeManager::refresh_window(player);
        RoomTradeManager::refresh_window(&trade_partner);

        if room_user.has_accepted_trade()
            && trade_partner
                .get_room_user()
                .map_or(false, |e| e.has_accepted_trade())
        {
            RoomTradeManager::add_items(player, &trade_partner);
            RoomTradeManager::add_items(&trade_partner, player);
