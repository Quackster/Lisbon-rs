//! Mirrors `net.h4bbo.lisbon.game.item.interactors.types.PetWaterBowlInteractor`.
...
pub struct PetWaterBowlInteractor {
    #[allow(dead_code)]
    trigger: GenericTrigger,
}

impl PetWaterBowlInteractor {
    /// Mirrors the `PetWaterBowlInteractor()` constructor.
    pub fn new() -> Self {
        Self {
            #[allow(dead_code)]
    trigger: GenericTrigger,
    ...
    pub fn on_item_pickup(&self, _player: &Player, room: &Room, item: &Item) {
        Self::cancel_pets_drinking(room, item.get_position());
    }
    ...
    pub fn on_item_moved(
        ...
        Self::cancel_pets_drinking(room, old_position);
    }

    /// Mirrors `cancelPetsDrinking(Room, Position)`.
    fn cancel_pets_drinking(room: &Room, position: &Position) {
...
impl Default for PetWaterBowlInteractor {
    fn default() -> Self {
        Self::new()
    }
}
