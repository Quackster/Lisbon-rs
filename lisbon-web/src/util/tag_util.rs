//! Mirrors `org.alexdev.http.util.TagUtil`.
...
impl TagUtil {
    /// Mirrors `getRandomQuestion()`.
    pub fn get_random_question() -> String {
        let questions = Self::get_random_questions();
        questions[rand::thread_rng().gen_range(0..questions.len())].clone()
    }

    /// Mirrors `getRandomQuestions()`.
    pub fn get_random_questions() -> Vec<String> {
        let string_list: [&str; 566] = [
            "Dog or Cat?",
            "Netflix or YouTube?",
            "Phone Call or Text?",
            "Toast or Eggs?",
